...
- `flushed`：仅 `status: "queued"` 且走客户端本地队列时存在，resolve `{ status: "delivered" | "expired" | "dropped" | "failed" }`；`delivered` 以对端 `__ack` 确认为准（对端 handler 已执行完），旧版本对端不回 `__ack` 时退化为传输层成功即 `delivered`；补投在结论明确前保留持久化记录，全部投递失败或 ACK 超时会自动重投直至送达或 TTL 过期。`via: "server"` 表示消息已存入服务端离线收件箱（对端重连即自动补投，跨刷新有效）
