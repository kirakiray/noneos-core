...
pub(crate) struct UserSession {
    ...
    pub(crate) data_tx: mpsc::Sender<Message>, // 用于转发消息的目标通道 (bounded)
...
pub struct AppState {
    ...
    pub(crate) watchers: DashMap<String, DashMap<String, mpsc::Sender<Message>>>,
    /// 反向索引：watcher_conn_key -> [watched_user_id]
    /// 用于 watcher 连接断开时清理所有关注关系
    pub(crate) watch_targets: DashMap<String, Vec<String>>,
    pub db_tx: Option<mpsc::Sender<crate::db_actor::DbCommand>>,
}

impl AppState {
    pub fn new(admin_user_id: Option<String>, config: Config, db_tx: Option<mpsc::Sender<crate::db_actor::DbCommand>>) -> Self {
        Self {
    ...
    pub fn add_user(&self, conn_key: &str, username: &str, host: &str, addr: SocketAddr, disconnect_tx: oneshot::Sender<()>, data_tx: mpsc::Sender<Message>) -> Result<(), String> {
        // 解析 userId
    ...
    pub fn add_watcher(&self, watcher_conn_key: &str, watched_user_id: &str, data_tx: mpsc::Sender<Message>) {
        ...
        if let Some(watchers) = self.watchers.get(user_id) {
            ...
            if let Ok(text) = serde_json::to_string(&notify) {
                for w in watchers.iter() {
                    let _ = w.value().try_send(Message::Text(text.clone()));
                }
            }
        }
    }
    ...
    pub fn get_session_data_tx(&self, user_id: &str, session_id: &str) -> Option<mpsc::Sender<Message>> {
    ...
    match verify_signature(&public_key, &signed_message, &signature) {
        Ok(_) => {
            ...
            if let Some(ref db_tx) = state.db_tx {
                ...
                let _ = db_tx.try_send(crate::db_actor::DbCommand::SaveUser(user_record));
            }
            ...
            let comm_result: std::result::Result<(), Box<dyn std::error::Error + Send + Sync>> = async {
                loop {
                    tokio::select! {
                        ...
                        // 接收客户端消息
                        msg_result = tokio::time::timeout(Duration::from_secs(300), ws_receiver.next()) => {
                            match msg_result {
                                Ok(Some(Ok(msg))) => {
                                    match msg {
                                        Message::Text(text) => {
                                                ...
                                                if action == "send_data" {
                                                    ...
                                                    if target_user.is_empty() || target_session.is_empty() || relay_data.is_none() {
                                                        ...
                                                        let delivered = if let Some(tx) = state.get_session_data_tx(target_user, target_session) {
                                                            tx.try_send(Message::Text(forward_text)).is_ok()
                                                        } else {
                                                            false
                                                        };
                                    ...
                                    Message::Binary(data) => {
                                        ...
                                        if msg_type == "relay" && action == "send_data" {
                                            ...
                                            if target_user.is_empty() || target_session.is_empty() {
                                                ...
                                                let delivered = if let Some(tx) = state.get_session_data_tx(target_user, target_session) {
                                                    tx.try_send(Message::Binary(forward_frame)).is_ok()
                                                } else {
                                                    false
                                                };
                                ...
                                Ok(None) => break,
                                Err(_) => {
                                    println!("Connection timeout (300s inactive) for {}:{}", user_id, session_id);
                                    let _ = ws_sender.send(Message::Close(None)).await;
                                    break;
                                }
                            }
            ...
            // 9. 无论通信循环是否出错，始终执行清理
            {
                ...
                if let Some(session) = state.traffic.lock().unwrap().remove_session(&conn_key) {
                    if let Some(ref db_tx) = state.db_tx {
                        let _ = db_tx.try_send(crate::db_actor::DbCommand::SaveFinalSession(session));
                    }
                }
                if let Some(quota) = state.user_quotas.get(&user_id) {
                    if let Some(ref db_tx) = state.db_tx {
                        let _ = db_tx.try_send(crate::db_actor::DbCommand::SaveFinalUserQuota(quota.clone()));
                    }
                }
            }
