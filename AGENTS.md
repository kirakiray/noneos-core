...
- **组件库**：项目目前深度集成 `punch-ui` 组件库，并将逐渐迁移至 `senti-ui`。
- **迁移方向**：两套 UI 组件库的规则与设计风格很相似，请优先使用 `senti-ui`（`st-*` 组件）；尚未迁移的存量页面仍基于 `punch-ui`，修改时应保持原有风格一致，不强制混用两套组件库。
- **视觉系统**：严格遵循所用组件库的颜色方案与设计语言。
- **开发参考**：在实现 UI 相关功能前，请查阅对应组件库的 Skill 知识库（`senti-ui` 或 `punch-ui`）以保持风格一致性。
