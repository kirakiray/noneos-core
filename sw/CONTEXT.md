...
│   └── modules/
│       ├── app-cache-handle.js  # 宿主项目应用缓存代理（OPFS 读取 + 路径 Set 拦截）
...
├── handleAppCacheRequest     (modules/app-cache-handle.js)   ← fetch 链末尾兜底
├── hasAppCachePath           (modules/app-cache-handle.js)   ← 路径匹配判断
└── initAppCachePaths         (modules/app-cache-handle.js)   ← activate/__config 时构建路径 Set
```
...
| `/__config` 路径 | 特殊路由：触发 `reloadSystemConfig()` 并返回 `{ serviceWorkerVersion, systemConfig, appCacheConfig }` JSON；`appCacheConfig` 来自宿主 sw.js 中的 `globalThis.NONEOS_APP_CACHE`（未配置时为 `null`） |
| `install` | `skipWaiting()` 立即激活 |
| `activate` | `clients.claim()` 接管页面，1s 后刷新配置 + `initAppCachePaths()` |
| `reloadSystemConfig()` | 从 OPFS `nos-config/system.json` 读取 `systemConfig`，同时调用 `initAppCachePaths()` 刷新应用缓存路径集合；失败返回 500 状态码 |
...
| （兜底）已缓存路径 | `app-cache-handle.js` | 宿主项目应用缓存；GET 请求路径命中内存 `cachedPaths` Set 时，从 OPFS `app-cache/` 读取；否则不拦截 |
...
- `/__config` 请求触发 `reloadSystemConfig()` 并返回当前版本与配置（含 `appCacheConfig`）；读取失败返回 500。
- `reloadSystemConfig()` 同时调用 `initAppCachePaths()` 刷新应用缓存路径集合。
...
### 7. 宿主项目应用缓存（app-cache-handle.js）
...
**工作原理**：
1. `/__config` 响应中携带 `appCacheConfig`（即 `globalThis.NONEOS_APP_CACHE`），页面侧通过 `check()` 获取。
2. `install()` 完成后自动调用 `installAppCacheIfConfigured()`，下载清单内所有文件写入 OPFS `app-cache/`。
3. SW activate / `/__config` 时调用 `initAppCachePaths()` 从 OPFS `app-cache/manifest.json` 构建内存路径 `Set`。
